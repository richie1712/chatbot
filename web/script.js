$(document).ready(function () {
	...
	var bot = '<div class="chatCont" id="chatCont">' +
		...
		'<img src="download.png" class="bot_p_img">' +
		'<div class="close">' +
		...
		'<img src="download.png" class="img-circle img-profile">' +
		'</div><!--col-hgt end-->' +
		'<div class="col-hgt">' +
		'<div class="chat-txt">' +
		'' +
	...
	function send(text) {
		...
		$.ajax({
			url: 'http://localhost:5005/conversations/admin/respond', //  RASA API
			type: 'POST',
			headers: {
			...
			error: function (xhr, textStatus, errorThrown) {
				console.log('There is Error in Operation');
	...
	function setBotResponse(val) {
		setTimeout(function () {
			...
			if ($.trim(val) == '' || val == 'error') { //if there is no response from bot or there is some error
				val = 'Sorry, I didn\'t get that ?????. Could you please rephrase?'
